...
The short answer is that since ~a~ is lowercase, unlike ~String~, it is an unbound /type variable/, and therefore that elm doesn't yet have enough information to say what type ~[]~ is.

We can actually give it this information explicitly by asserting the type of ~[]~.
...
#+END_SRC
Often times source code would become so buried in types that it became hard to
read. Then, in 1973, the programming language ML was released, featuring the
